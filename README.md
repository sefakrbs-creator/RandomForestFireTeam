...
A **shrub detection pipeline** that produces georeferenced shrub probability maps for aerial (NAIP) imagery across 6 California field sites. Ground truth labels come from SAM polygon annotations on 5-channel (NAIP + CHM) patches derived from Terrestrial LiDAR Scans (TLS).
...
├── download_naip_all_sites.py       # Download NAIP from Planetary Computer
├── download_canopy_height_all_sites.py  # Download CHM from Google Earth Engine
├── generate_all_sam_viz.py          # Generate PNG visualizations for ALL SAM tiles
│
├── Model_Prediction/
...
Python >= 3.10 (tested on 3.12 under WSL/Ubuntu).

```bash
pip install -r requirements.txt
```

SAM must be installed separately:

```bash
pip install git+https://github.com/facebookresearch/segment-anything.git
```

Then download the SAM ViT-H checkpoint (~2.5 GB):

```
sam_vit_h_4b8939.pth  — from Meta's SAM release
```

> All scripts are designed to run under WSL (Ubuntu). Activate the venv with:
...
Exports all dataset patches as PNGs for manual review in Label Studio **before SAM refinement**.
...
label-studio start
# Create project → Import → label_studio_images/label_studio_import.json
```
...
Refines raw circular shrub annotations and mines hard negatives using Segment Anything Model (SAM).
...
- Area < 4 px² or > 3000 px² → skip

### Key constants
| Constant | Value | Description |
|---|---|---|
| `UPSCALE` | 4 | Patches upscaled 128→512 px before SAM |
...
Labels stored in COCO instance format:
...
3. **Subsample negatives** to MAX_NEG_PER_PATCH=200 per patch
4. **Pass 1** — fit XGBoost on all 55 features, rank by importance, select top 20
5. **Pass 2** — refit RobustScaler on top-20 features; 80-trial Optuna NSGA-II search maximising F1/Precision/Recall; Pareto-front best selected by F1
6. **Final model** — retrain with best params, n_estimators=2000, early_stopping_rounds=50

### Top 20 selected features (from `v12_model_meta.json`)
...
> **Scaling note:** `canopy_in_shrub_range` is a binary 0/1 flag in `no_scale_features` — it must NOT be passed through RobustScaler. Always check `v12_model_meta.json` → `"no_scale_features"` before applying the scaler.
...
- NAIP 4-band TIF (R, G, B, NIR)
- Canopy height TIF (CHM)
...
CRS: EPSG:6350 (NAD83(2011) / Conus Albers).
...
Standalone inference module with a single entry point:
...
End-to-end pipeline notebook with 8 sections.

| # | Section | Description |
|---|---|---|
| 1 | Logging Setup | Creates `logs/{RUN_TS}_pipeline.log` |
...
| Group | Packages |
|---|---|
| Core data & ML | numpy, pandas, scipy, scikit-learn, xgboost, optuna, joblib |
| Geospatial | rasterio, affine, fiona, shapely, pyproj, geopandas |
| Remote sensing & download | earthengine-api, geemap, google-cloud-storage, google-auth, planetary-computer, pystac-client, requests |
| Computer vision & annotation | opencv-python, torch, torchvision, Pillow |
| Visualisation | matplotlib |
| Notebook / labeling | jupyter, notebook, label-studio |
# RandomForestFireTeam
